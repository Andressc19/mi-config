...
local function get_system_node()
  ...
  local system_paths = {
    ...
    -- NVM for Windows (try latest versions first)
    "C:/Users/joseb/AppData/Roaming/nvm/v22.14.0/node.exe",
    "C:/Users/joseb/AppData/Roaming/nvm/v21.7.3/node.exe",
    "C:/Users/joseb/AppData/Roaming/nvm/v20.19.0/node.exe",
    -- Standard Windows Node.js installation
...
local function setup_nodejs()
  ...
  if node_path ~= "" then
    -- Use vim.fn.systemlist to properly handle paths with spaces (Windows)
    local version_output = ""
    local success = false
    
    -- Try with systemlist first (better for Windows)
    local result = vim.fn.systemlist({ node_path, "--version" })
    if vim.v.shell_error == 0 and #result > 0 then
      version_output = result[1]
      success = true
    end
    
    -- Fallback to system with shell escaping for Unix-style systems
    if not success then
      version_output = vim.fn.system(node_path .. " --version 2>/dev/null")
...
function M.info()
  ...
  local result = vim.fn.systemlist({ vim.g.node_host_prog, "--version" })
  if vim.v.shell_error ~= 0 or #result == 0 then
    print("Node.js: Error getting version")
    return
  end

  local version = result[1]:gsub("\r", ""):gsub("\n", ""):gsub("v", "")
